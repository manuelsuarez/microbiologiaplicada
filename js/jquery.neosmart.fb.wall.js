...
//mod cuervo: cortar el mensaje a cierto numero de caracteres
	var $mensaje=data[k].message;
	$mensaje=$mensaje.substring(0, 60)+"...";
					...
					output += '<a href="http://www.facebook.com/profile.php?id='+data[k].from.id+'" class="fb-wall-message-from" target="_blank">'+data[k].from.name+'</a><br> ';
					if(exists($mensaje)) output += modText($mensaje);
					output += '</span>';
				...
				//MOD CUERVo
				//ajustar tamaño al cargar contenido
				var altura= $("body").height()-76-10-48-20;
				$("#facebook-posts").css({height:altura+"px"});
				$("#facebook-posts").slideDown(100);
			}
